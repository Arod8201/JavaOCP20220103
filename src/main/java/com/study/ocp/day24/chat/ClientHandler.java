...
/**
 * When a client connects the server spawns a thread to handle the client. This
 * way the server can handle multiple clients at the same time.
 *
...
public class ClientHandler implements Runnable {
	...
	// Array list of all the threads handling clients so each message can be sent to
	// the client the thread is handling.
	public static ArrayList<ClientHandler> clientHandlers = new ArrayList<>();
	// Id that will increment with each new client.

	// Socket for a connection, buffer reader and writer for receiving and sending
	// data respectively.
	...
	// blocking operation.
	// A blocking operation means the caller waits for the callee to finish its
	// operation.
	@Override
	public void run() {
	...
	// Send a message through each client handler thread so that everyone gets the
	// message.
	// Basically each client handler is a connection to a client. So for any message
	// that
	...
	// If the client disconnects for any reason remove them from the list so a
	// message isn't sent down a broken connection.
	...
	public void closeEverything(Socket socket, BufferedReader bufferedReader, BufferedWriter bufferedWriter) {
		...
		// The client disconnected or an error occurred so remove them from the list so
		// no message is broadcasted.
		removeClientHandler();
		try {
